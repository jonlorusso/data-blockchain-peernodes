...
public class ChainNodeManager {
	...
	public ChainNode getChainNode(String name) throws OperationFailedException {
		ChainNode chainNode = chainNodes.get(name);
		
		if (chainNode == null) {
			ChainNodeConfig chainNodeConfig = chainNodeManagerConfig.getChainNodeConfig(name);
			
			if (chainNodeConfig == null) {
				throw new OperationFailedException("No ChainNodeConfig found for: " + name);
			}
			...
			try {
				...
				throw new OperationFailedException("Unable to create ChainNode: " + name, t);
			}
		}
