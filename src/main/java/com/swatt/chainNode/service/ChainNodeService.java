...
public class ChainNodeService {
	...
	public ChainNodeService(final ChainNodeManager chainNodeManager, int port, final ConnectionPool connectionPool) {
        ...
        app.get("/:chainName/txn/:transactionHash", ctx -> {
        	...
        	Transaction chainTransaction = chainNode.fetchTransactionByHash(transactionHash, calculateFees);			// Fetch the transactions and calculate fees
        	
            String result = JsonUtilities.objectToJsonString(chainTransaction);
            ctx.result(result);
        });
        
        app.get("/:chainName/fetch/:blockHash", ctx -> {					// TODO:  This is just for debugging, this is not meant to be part of the Service API.
        	String chainName = ctx.param("chainName");
        	String blockHash = ctx.param("blockHash");
        	
        	Connection conn = connectionPool.getConnection();	// Do not use the JDK 1.7+ try with resource as we do NOT want to close the pooled connections 
        ...
        app.get("/:chainName/blk/:blockHash", ctx -> {
        	String chainName = ctx.param("chainName");
        	String blockHash = ctx.param("blockHash");
        ...
        app.get("/:chainName/chn/:from/:to", ctx -> {
        	String chainName = ctx.param("chainName");
...
//            ctx.result(returnObject(data));
    ...
    public static void main(String[] args) {
    	try {
	    	...
	    	int port = chainNodeManagerConfig.getIntAttribute("servicePort", 0);
	    	...
	    	if (port > 0) {
		        ...
		        ConcurrencyUtilities.startAutoDestructTimer(autoExitTimeout);		// This is useful while debugging so you don't have to constantly stop server to restart it
	    	}
    	} catch (Throwable t) {
    		t.printStackTrace();
    	}
