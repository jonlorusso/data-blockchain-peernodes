...
public class BitcoinChainNode extends ChainNode {
	...
	@Override
	public void init() {
//		String url = chainNodeConfig.getAttribute("xxx", null);				// TODO: Should get from chainNodeConfig
//		String user = chainNodeConfig.getAttribute("xxx", null);			// TODO: Should get from chainNodeConfig
//		String password = chainNodeConfig.getAttribute("xxx", null);		// TODO: Should get from chainNodeConfig
//		int maxSize = chainNodeConfig.getAttribute("xxx", null);	// TODO: Should get from chainNodeConfig
		
		String url = "http://127.0.0.1";									// TODO: Should get from chainNodeConfig
		String user = "christian";											// TODO: Should get from chainNodeConfig
		String password = "thePassword";									// TODO: Should get from chainNodeConfig
    ...
    @Override
    public Transaction fetchTransactionByHash(String transactionHash, boolean calculateFee) throws OperationFailedException {
		JsonRpcHttpClient jsonRpcHttpClient = jsonRpcHttpClientPool.getJsonRpcHttpClient();
    ...
    private BlockData fetchLatestBlock(JsonRpcHttpClient jsonRpcHttpClient) throws OperationFailedException {
        ...
        return fetchBlockByBlockNumber(jsonRpcHttpClient, blockNumber);	// We keep this out of the above try/catch so we don't double catch exceptions on this call
    }
    ...
    private BlockData fetchBlockByBlockNumber(JsonRpcHttpClient jsonrpcClient, long blockNumber) throws OperationFailedException {
        ...
        return fetchBlockByHash(jsonrpcClient, blockHash);	// We keep this out of the above try/catch so we don't double catch exceptions on this call
    }
    ...
    private void calculate(JsonRpcHttpClient jsonrpcClient, BlockData blockData, RPCBlock rpcBlock) throws OperationFailedException {
